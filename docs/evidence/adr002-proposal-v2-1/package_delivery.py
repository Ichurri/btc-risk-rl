...
def main():
    ...
    stem = f'btc-risk-rl-adr002-v2-1-{commit[:7]}'
    ...
    readme = '''# Entrega académica ADR-002 v2.1

Estado: PROPUESTA PARA REVISIÓN, NO ADOPTADA. ADR-002 sigue abierto.
Leer docs/proposals/ADR-002-v2-1-resumen-academico.md y ADR-002-propuesta-v2-1.md.
El pseudocódigo completo está en la sección 6.4 de la propuesta.
Evidencia nueva: docs/evidence/adr002-proposal-v2-1/.
...
Para reproducir solo los cálculos (Python 3.12.13 usado originalmente):
  git clone repository.bundle revision-adr002-v2-1
  cd revision-adr002-v2-1
  python3 docs/evidence/adr002-proposal-v2-1/checks.py > /tmp/adr002-v2-1-checks.json
Los checks no escriben antecedentes ni leen datos de mercado.
Para repetir H2 se necesitan las dependencias fijadas por uv.lock:
  uv sync --frozen
  uv run --frozen ruff check .
  uv run --frozen pytest
No ejecutar fetch-development, verify_simulator ni entrenamientos para revisar v2-1.
...
'''
    ...
    manifest = {
        ...
        'packaging_command': 'uv run --frozen python docs/evidence/adr002-proposal-v2-1/package_delivery.py',
