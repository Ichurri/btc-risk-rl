# Propuesta ADR-002 v2.1: retorno finito común y riesgo de cola
...
Revisión de armonización sobre v2 (`28cda8b`), conservada íntegra como antecedente.
La revisión académica comunicada por el usuario da por resueltos Q/A/B y el
alcance de transferencia operacional; esta v2.1 solo armoniza métrica e inferencia.
...
### 7.2 Métrica primaria y plan inferencial de la tesis

El contraste principal compara el **Sortino anualizado fuera de muestra** de
los procedimientos completos C5/C10 con C0, incluyendo la regla común de
despliegue y presupuesto. No verifica la cota CVaR ni estima directamente J.
El desplazamiento de soporte puede interactuar de forma diferente con cada
condición; una regla común no cancela ese efecto. Una mejora apoyaría transferencia
operacional en el período y semillas evaluados, sin identificar el control de
cola como causa aislada ni demostrar cumplimiento CVaR a 30 días.

Con u_t=E_(t+1)/E_t−1, retornos simples netos de 4 h, MAR m=0 por período:

\[
DD=\sqrt{\frac1T\sum_{t=0}^{T-1}\min(u_t-m,0)^2},\qquad
S_{4h}=\frac{T^{-1}\sum_t(u_t-m)}{DD},\qquad
\boxed{S_{anualizado}=\sqrt{2190}\,S_{4h}}.
\]

El denominador incluye **todos** los períodos, no solo los negativos. Se conserva
S_4h como resultado auxiliar. El factor fijo sqrt(2190), correspondiente a seis
períodos diarios por 365 días, es la **convención de reporte de la tesis**. No
supone independencia temporal ni demuestra que el riesgo de un retorno anual
agregado escale con esa raíz; no se sustituye por sqrt(T) según la longitud de
la muestra. MAR cero no es d ni un cuantil. Si DD=0, ambos ratios son indefinidos;
registrar numerador y DD, sin epsilon, imputación a cero ni victoria por infinito.

Ejemplo sintético: (.03,−.01,.01,−.02) da DD=sqrt(.000125),
S_4h≈.2236068 y S_anualizado=sqrt(109.5)≈10.4642248. Con solo cuatro observaciones
es únicamente una comprobación aritmética, no evidencia de precisión estadística.

#### Plan inferencial ya propuesto en la tesis

Se incorpora el procedimiento indicado por el usuario en esta revisión; no
se presenta como selección nueva a partir de validación. En el repositorio v2
no había una especificación autónoma de bootstrap/Holm; esta sección deja la
vinculación registrada, sin modificar el documento de tesis.

Sea b=1,...,n un **bloque de semillas emparejado**, con una réplica completa por
condición y las mismas reglas de evaluación. Formar primero:

\[
D_{b,5}=S^{anualizado}_{b,C5}-S^{anualizado}_{b,C0},\quad
D_{b,10}=S^{anualizado}_{b,C10}-S^{anualizado}_{b,C0},\quad
\bar D_c=\frac1n\sum_b D_{b,c}.
\]

Para cada c∈{5,10}, contrastar unilateralmente H0_c: E[D_c]≤0 frente a
H1_c: E[D_c]>0. La frontera nula se impone centrando
Z_(b,c)=D_(b,c)−bar D_c. En cada remuestra r, sortear n índices de bloque con
reemplazo y usar **los mismos índices para las dos columnas**. Así se conserva
el emparejamiento de condiciones y el C0 compartido. Calcular
T*_(r,c)=mean_b Z_(I_(r,b),c) y comparar su cola superior con bar D_c observada.
Una aproximación Monte Carlo explícita es

\[
\widehat p_c=\frac{1+\sum_{r=1}^{B}\mathbf1\{T^*_{r,c}\ge\bar D_c\}}{B+1}.
\]

La corrección de conteo evita p=0 por resolución finita; no convierte el bootstrap
en un test exacto. Si se enumeran todas las remuestras de un ejemplo pequeño,
el oráculo usa su fracción exacta de excedencias, sin esa corrección Monte Carlo.
No usar el bootstrap sin centrar como distribución nula ni remuestrear cada
condición por separado. No usar cambios de signo como sustituto de este plan.

Aplicar **Holm** a la familia de dos contrastes con significancia familiar
alpha_F=0.05 (distinta de las fracciones de cola .05/.10 del entrenamiento).
Ordenar p_(1)≤p_(2): rechazar el primero solo si p_(1)≤.025 y, únicamente si se
rechazó, el segundo si p_(2)≤.05. Detenerse en el primer no rechazo. Reportar
p brutos y ajustados, mapeados a sus contrastes originales:

\[
p^{Holm}_{(1)}=\min(1,2p_{(1)}),\qquad
p^{Holm}_{(2)}=\min(1,\max(2p_{(1)},p_{(2)})).
\]

La interpretación del control familiar depende de la validez de los p marginales;
Holm admite dependencia entre contrastes, pero no corrige un bootstrap mal
especificado ni garantiza calibración exacta con pocas réplicas o ratios inestables.
Los bloques de semillas deben constituir unidades de replicación válidas; el
remuestreo sobre ellos no requiere independencia entre velas dentro de cada
recorrido, pero tampoco captura por sí solo incertidumbre sobre otros períodos
económicos. No considerar las 2190 velas ni ventanas solapadas réplicas independientes.

El factor sqrt(2190) multiplica todas las diferencias por la misma constante
positiva: cambia la escala de efectos reportados, no el orden de las excedencias
bootstrap ni la decisión de Holm usando las mismas remuestras y precisión adecuada.
Una anualización convencional no crea evidencia nueva ni una garantía CVaR.

```text
Antes de evaluar: congelar n, B, semillas/bloques, precisión y política de indefinidos.
Obtener un Sortino anualizado por réplica/condición, conservando el emparejamiento.
Si hay indefinidos: aplicar únicamente el protocolo previamente cerrado;
    mientras ese protocolo esté pendiente, no emitir un contraste confirmatorio.
D <- diferencias pareadas C5-C0 y C10-C0; efecto <- media por columna.
Z <- D - efecto por columna (frontera nula de media cero).
Para cada remuestra: elegir bloques con reemplazo y usar sus dos columnas juntas;
    T* <- medias de Z remuestreado.
p <- cola superior respecto del efecto observado, con conteo Monte Carlo indicado.
Aplicar Holm a ambos p, alpha familiar=.05; reportar efectos, p brutos y ajustados.
```

#### Parámetros pendientes, sin reabrir el procedimiento

Están propuestos: diferencias pareadas por bloques de semillas, media de
diferencias, bootstrap unilateral centrado bajo H0 y Holm para esos dos
contrastes con alpha_F=.05. **Faltan cerrar antes de evaluar confirmatoriamente**:
número de réplicas n, remuestras B, semillas y composición de bloques, precisión
numérica/Monte Carlo y tratamiento de métricas indefinidas (incluyendo qué hacer
con bloques incompletos y su impacto en el estimando). No eliminar indefinidos
selectivamente después de ver qué condición favorecen ni reemplazarlos por cero.
La regla DD=0→indefinido ya está fijada; la gestión inferencial de esos casos
sigue pendiente. Los ejemplos sintéticos no fijan tamaños de una campaña real.

No seleccionar reloj, d, checkpoint, fórmula o parámetros inferenciales tras
ver validación. Si 2023 se utiliza para elegir hiperparámetros, su contraste es
de desarrollo, no confirmatorio independiente. La prueba final seguirá protegida
hasta cerrar el protocolo y obtener una autorización separada.
...
| Estructura propuesta (v2 y armonización v2.1) | Pendiente de pilotos/protocolo previo a comparación |
|---|---|
...
| Igual presupuesto incluyendo auxiliares, regla de evaluación idéntica | Número de réplicas, remuestras, semillas/bloques, precisión y gestión inferencial de indefinidos |
| Sortino anualizado primario, pareamiento/bootstrap centrado/Holm .05; h=1 y cartera única | Protocolo confirmatorio futuro; validación usada en selección se declara de desarrollo |
...
## 10. Antecedente v2 y evidencia de armonización v2.1

La evidencia nueva de armonización está en
[checks v2.1](../evidence/adr002-proposal-v2-1/checks.py),
[resultados](../evidence/adr002-proposal-v2-1/results.json) y
[comandos](../evidence/adr002-proposal-v2-1/COMMANDS.md).
Comprueba anualización/indefinidos, bootstrap pareado centrado y Holm con datos
sintéticos; no ejecuta una evaluación ni implementa el módulo de evaluación.
Verifica además que §6 (Q/A/B), §7.1 y §7.3 (regla temporal) coinciden byte a byte
con v2, y que el contrato H2 permanece intacto. Los resultados que siguen son
**antecedentes de v2**, no ejecuciones nuevas atribuibles a v2.1.
...
8. [Nassar y Ephrem (2020), §2, investigación original](https://arxiv.org/pdf/2007.06460).
   ...
   discreta, frecuencia y MAR cero son convenciones explícitas; v2.1 añade
   la anualización de reporte solicitada por la tesis;
...
convención de Sortino (armonizada en v2.1) y prohibición de selección retrospectiva.
...
Siguiente paso: revisión de la armonización v2.1 y cierre de parámetros pendientes. ADR-002 permanece abierto; ninguna
propuesta de este documento cambia el contrato H2 ni autoriza entrenamientos.

### Referencia añadida para la armonización

[Holm (1979), A Simple Sequentially Rejective Multiple Test Procedure](https://www.ime.usp.br/~abe/lista/pdf4R8xPVzCnX.pdf)
fundamenta el procedimiento secuencial de multiplicidad. El plan de bootstrap
pareado centrado se incorpora de la instrucción académica del usuario; el
conteo Monte Carlo se explicita para reproducibilidad. Ningún resultado de
Holm convierte los ejemplos sintéticos o la anualización en garantía CVaR.
